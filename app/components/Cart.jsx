...
export function CartDetails({layout, cart}) {
  ...
  return (
    ...
    //     </div>
    //     <div className='sm:flex-1 sm:mt-4 md:mt-4 md:flex-1 flex-0'>
    //     {cartHasItems && (
    ...
    >
      <ul className="grid gap-6 md:gap-10">
        {currentLines.map((line) => (
          <CartLineItem2 key={line.id} line={line} />
        ))}
      </ul>
    </section>
  );
...
function CartSummary({cost, layout, children = null}) {
  const summary = {
    ...
    page: 'sticky top-nav grid gap-6 p-4 md:px-6 md:translate-y-4 lg:translate-y-12 bg-primary/5 rounded w-full',
  };
